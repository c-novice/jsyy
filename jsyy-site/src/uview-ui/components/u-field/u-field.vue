<template>
  <view :class="{'u-border-top': borderTop, 'u-border-bottom': borderBottom }" class="u-field">
    <view :class="[type == 'textarea' ? 'u-textarea-inner' : '', 'u-label-postion-' + labelPosition]"
          class="u-field-inner">
      <view :class="[required ? 'u-required' : '']" :style="{
				justifyContent: justifyContent,
				flex: labelPosition == 'left' ? `0 0 ${labelWidth}rpx` : '1'
			}" class="u-label">
        <view v-if="icon" class="u-icon-wrap">
          <u-icon :color="iconColor" :custom-style="iconStyle" :name="icon" class="u-icon" size="32"></u-icon>
        </view>
        <slot name="icon"></slot>
        <text :class="[this.$slots.icon || icon ? 'u-label-left-gap' : '']" class="u-label-text">{{ label }}</text>
      </view>
      <view class="fild-body">
        <view :style="[inputWrapStyle]" class="u-flex-1 u-flex">
					<textarea v-if="type == 'textarea'" :autoHeight="autoHeight" :disabled="disabled" :fixed="fixed"
        ...
        <u-icon v-if="clearable && value != '' && focused" :size="clearSize" class="u-clear-icon" color="#c0c4cc"
                name="close-circle-fill" @click="onClear"/>
        <view class="u-button-wrap">
          <slot name="right"/>
        </view>
        <u-icon v-if="rightIcon" :name="rightIcon" :style="[rightIconStyle]" class="u-arror-right" color="#c0c4cc"
                size="26" @click="rightIconClick"/>
      </view>
    </view>
    <view v-if="errorMessage !== false && errorMessage != ''" :style="{
			paddingLeft: labelWidth + 'rpx'
		}" class="u-error-message">{{ errorMessage }}
    </view>
  </view>
</template>
