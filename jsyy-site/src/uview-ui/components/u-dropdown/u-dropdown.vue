<template>
  <view class="u-dropdown">
    <view :class="{
      ...
      <view v-for="(item, index) in menuList" :key="index" class="u-dropdown__menu__item" @tap.stop="menuClick(index)">
        <view class="u-flex">
          <text :style="{
						color: item.disabled ? '#c0c4cc' : (index === current || highlightIndex == index) ? activeColor : inactiveColor,
						fontSize: $u.addUnit(titleSize)
					}" class="u-dropdown__menu__item__text">{{ item.title }}
          </text>
          <view :class="{
    ...
    <view :style="[contentStyle, {
      ...
      <view :style="[popupStyle]" class="u-dropdown__content__popup" @tap.stop.prevent>
        <slot></slot>
      </view>
