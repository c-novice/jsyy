<template>
  <view :style="[itemStyle]" class="u-collapse-item">
    <view :hover-class="hoverClass" :hover-stay-time="200" :style="[headStyle]" class="u-collapse-head"
          @tap.stop="headClick">
      <block v-if="!$slots['title-all']">
        <view v-if="!$slots['title']" :style="[{ textAlign: align ? align : 'left' },
					isShow && activeStyle && !arrow ? activeStyle : '']" class="u-collapse-title u-line-1">
          {{ title }}
        </view>
        <slot v-else name="title"/>
        <view class="u-icon-wrap">
          <u-icon v-if="arrow" :class="{ 'u-arrow-down-icon-active': isShow }" :color="arrowColor"
    ...
    <view :style="[{
      ...
      <view :id="elId" :style="[bodyStyle]" class="u-collapse-content">
        <slot></slot>
      </view>
    </view>
  </view>
