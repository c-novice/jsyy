<template>
  <view>
    <u-popup v-model="value" :border-radius="borderRadius" :length="width" :mask-close-able="maskCloseAble" :negative-top="negativeTop" :popup="false"
             ...
             @close="popupClose">
      <view class="u-model">
        <view v-if="showTitle" :style="[titleStyle]" class="u-model__title u-line-1">{{ title }}</view>
        <view class="u-model__content">
          <view v-if="$slots.default  || $slots.$default" :style="[contentStyle]">
            <slot/>
          </view>
          <view v-else :style="[contentStyle]" class="u-model__content__message">{{ content }}</view>
        </view>
        <view v-if="showCancelButton || showConfirmButton" class="u-model__footer u-border-top">
          <view v-if="showCancelButton" :hover-stay-time="100" :style="[cancelBtnStyle]"
          ...
          <view v-if="showConfirmButton || $slots['confirm-button']" :hover-class="asyncClose ? 'none' : 'u-model__btn--hover'"
            ...
            <block v-else>
              <u-loading v-if="loading" :color="confirmColor" mode="circle"></u-loading>
              <block v-else>
                {{ confirmText }}
              </block>
            </block>
