<template>
  <view :class="{'u-border-bottom': elBorderBottom, 'u-form-item__border-bottom--error': validateState === 'error' && showError('border-bottom')}"
      ...
      <view :style="{
        ...
        <view v-if="required || leftIcon || label" class="u-form-item--left__content">
          ...
          <view v-if="leftIcon" class="u-form-item--left__content__icon">
            <u-icon :custom-style="leftIconStyle" :name="leftIcon"></u-icon>
          </view>
          <view :style="[elLabelStyle, {
    ...
    <view v-if="validateState === 'error' && showError('message')" :style="{
			paddingLeft: elLabelPosition == 'left' ? $u.addUnit(elLabelWidth) : '0',
		}" class="u-form-item__message">{{ validateMessage }}
    </view>
  </view>
</template>
