<template>
  <view :style="{
    ...
    <view :id="id">
      <scroll-view :scroll-left="scrollLeft" class="u-scroll-view" scroll-with-animation scroll-x>
        <view :class="{'u-tabs-scorll-flex': !isScroll}" class="u-scroll-box">
          <view v-for="(item, index) in list" :id="'u-tab-item-' + index" :key="index" :style="[tabItemStyle(index)]"
                class="u-tab-item u-line-1"
                @tap="clickTab(index)">
            <u-badge :count="item[count] || item['count'] || 0" :offset="offset" size="mini"></u-badge>
            {{ item[name] || item['name'] }}
          </view>
          <view v-if="showBar" :style="[tabBarStyle]" class="u-tab-bar"></view>
        </view>
      </scroll-view>
    </view>
  </view>
