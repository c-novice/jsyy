<template>
  <view :class="'interlayer '+(c||'')" :style="s">
    <block v-for="(n, i) in nodes" v-bind:key="i">
      <!--图片-->
      <view v-if="n.name=='img'" :class="'_img '+n.attrs.class" :data-attrs="n.attrs" :style="n.attrs.style"
            @tap.stop="imgtap">
      ...
      <audio v-else-if="n.name=='audio'" :ref="n.attrs.id" :author="n.attrs.author" :autoplay="n.attrs.autoplay"
             :class="n.attrs.class"
             :controls="n.attrs.controls" :data-i="i" :data-id="n.attrs.id" :loop="n.attrs.loop"
             :name="n.attrs.name"
             :poster="n.attrs.poster" :src="n.attrs.source[ctrl[i]||0]" :style="n.attrs.style" data-source="audio"
             @error.native="error"
             @play.native="play"/>
      <!--链接-->
      <view v-else-if="n.name=='a'" :id="n.attrs.id" :class="'_a '+(n.attrs.class||'')" :data-attrs="n.attrs"
            :style="n.attrs.style"
            hover-class="_hover" @tap.stop="linkpress">
      ...
      <!--表格-->
      <view v-else-if="n.name=='table'&&n.c&&n.flag" :id="n.attrs.id" :class="n.attrs.class"
            :style="(n.attrs.style||'')+';display:grid'">
        <trees v-for="(cell,n) in n.children" v-bind:key="n" :c="cell.attrs.class" :class="cell.attrs.class"
               :nodes="cell.children"
               :s="cell.attrs.style" :style="cell.attrs.style"/>
      </view>
      <view v-else-if="n.name=='table'&&n.c" :id="n.attrs.id" :class="n.attrs.class"
        ...
        <view v-for="(tbody, o) in n.children" v-bind:key="o" :class="tbody.attrs.class"
          ...
          <view v-for="(tr, p) in tbody.children" v-bind:key="p" :class="tr.attrs.class"
            ...
            <trees v-for="(td, q) in tr.children" v-else v-bind:key="q" :c="td.attrs.class" :class="td.attrs.class"
                   :nodes="td.children"
      ...
      <!--#endif-->
      <trees v-else :c="(n.attrs.id||'')+' _'+n.name+' '+(n.attrs.class||'')"
             :class="(n.attrs.id||'')+' _'+n.name+' '+(n.attrs.class||'')"
             :lazyLoad="lazyLoad" :loading="loading" :nodes="n.children" :s="n.attrs.style" :style="n.attrs.style"/>
    </block>
  </view>
</template>
