<template>
  <u-popup v-model="value" :mask="mask" :maskCloseAble="maskCloseAble" :popup="false" :safeAreaInsetBottom="safeAreaInsetBottom" :zIndex="uZIndex"
    ...
    <slot/>
    <view v-if="tooltip" class="u-tooltip">
      ...
      <view v-if="confirmBtn" class="u-tooltip-item u-tooltips-submit" hover-class="u-tooltips-submit-hover"
            @tap="onConfirm">
        {{ confirmBtn ? confirmText : '' }}
      </view>
    </view>
    <block v-if="mode == 'number' || mode == 'card'">
      <u-number-keyboard :dotEnabled="dotEnabled" :mode="mode" :random="random" @backspace="backspace"
                         @change="change"></u-number-keyboard>
    </block>
    <block v-else>
