  ...
  <view :style="{
    ...
    <swiper :autoplay="autoplay" :circular="circular" :current="elCurrent" :duration="duration"
      ...
      <swiper-item v-for="(item, index) in list" :key="index" class="u-swiper-item">
        <view :class="[uCurrent != index ? 'u-list-scale' : '']" :style="{
						borderRadius: `${borderRadius}rpx`,
						transform: effect3d && uCurrent != index ? 'scaleY(0.9)' : 'scaleY(1)',
						margin: effect3d && uCurrent != index ? '0 20rpx' : 0,
					}"
              class="u-list-image-wrap" @tap.stop.prevent="listClick(index)">
          <image :mode="imgMode" :src="item[name] || item" class="u-swiper-image"></image>
          <view v-if="title && item.title" :style="[{
							'padding-bottom': titlePaddingBottom
						}, titleStyle]" class="u-swiper-title u-line-1">
            {{ item.title }}
          </view>
        </view>
      </swiper-item>
