  ...
  <u-popup v-model="value" :maskCloseAble="maskCloseAble" :popup="false" :safeAreaInsetBottom="safeAreaInsetBottom" :z-index="uZIndex"
    ...
    <view class="u-datetime-picker">
      ...
      <view class="u-picker-body">
        <picker-view v-if="mode == 'region'" :value="valueArr" class="u-picker-view" @change="change"
                     @pickend="pickend" @pickstart="pickstart">
          <picker-view-column v-if="!reset && params.province">
            <view v-for="(item, index) in provinces" :key="index" class="u-column-item">
              <view class="u-line-1">{{ item.label }}</view>
            </view>
          </picker-view-column>
          <picker-view-column v-if="!reset && params.city">
            <view v-for="(item, index) in citys" :key="index" class="u-column-item">
              <view class="u-line-1">{{ item.label }}</view>
            </view>
          </picker-view-column>
          <picker-view-column v-if="!reset && params.area">
            <view v-for="(item, index) in areas" :key="index" class="u-column-item">
              <view class="u-line-1">{{ item.label }}</view>
            </view>
          </picker-view-column>
        </picker-view>
        <picker-view v-else-if="mode == 'time'" :value="valueArr" class="u-picker-view" @change="change"
                     @pickend="pickend" @pickstart="pickstart">
          <picker-view-column v-if="!reset && params.year">
            <view v-for="(item, index) in years" :key="index" class="u-column-item">
              {{ item }}
              <text v-if="showTimeTag" class="u-text">年</text>
            </view>
          </picker-view-column>
          <picker-view-column v-if="!reset && params.month">
            <view v-for="(item, index) in months" :key="index" class="u-column-item">
              {{ formatNumber(item) }}
              <text v-if="showTimeTag" class="u-text">月</text>
            </view>
          </picker-view-column>
          <picker-view-column v-if="!reset && params.day">
            <view v-for="(item, index) in days" :key="index" class="u-column-item">
              {{ formatNumber(item) }}
              <text v-if="showTimeTag" class="u-text">日</text>
            </view>
          </picker-view-column>
          <picker-view-column v-if="!reset && params.hour">
            <view v-for="(item, index) in hours" :key="index" class="u-column-item">
              {{ formatNumber(item) }}
              <text v-if="showTimeTag" class="u-text">时</text>
            </view>
          </picker-view-column>
          <picker-view-column v-if="!reset && params.minute">
            <view v-for="(item, index) in minutes" :key="index" class="u-column-item">
              {{ formatNumber(item) }}
              <text v-if="showTimeTag" class="u-text">分</text>
            </view>
          </picker-view-column>
          <picker-view-column v-if="!reset && params.second">
            <view v-for="(item, index) in seconds" :key="index" class="u-column-item">
              {{ formatNumber(item) }}
              <text v-if="showTimeTag" class="u-text">秒</text>
            </view>
          </picker-view-column>
        </picker-view>
        <picker-view v-else-if="mode == 'selector'" :value="valueArr" class="u-picker-view" @change="change"
                     @pickend="pickend" @pickstart="pickstart">
          <picker-view-column v-if="!reset">
            <view v-for="(item, index) in range" :key="index" class="u-column-item">
              <view class="u-line-1">{{ getItemValue(item, 'selector') }}</view>
            </view>
          </picker-view-column>
        </picker-view>
        <picker-view v-else-if="mode == 'multiSelector'" :value="valueArr" class="u-picker-view" @change="change"
                     @pickend="pickend" @pickstart="pickstart">
          <picker-view-column v-for="(item, index) in range" v-if="!reset" :key="index">
            <view v-for="(item1, index1) in item" :key="index1" class="u-column-item">
